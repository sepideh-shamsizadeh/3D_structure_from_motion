...
void BasicSfM::solve()
{
    ...
    while( !seed_found )
    {
        ...
        std::cout<<"inlier mask E"<< inlier_mask_E.rows<< " " << inlier_mask_E.cols<<endl;
        std::cout<<inlier_mask_E<<endl;
        std::cout<<"inlier mask H"<< inlier_mask_H.rows<< " " << inlier_mask_H.cols<<endl;
        std::cout<<inlier_mask_H<<endl;

        int sum_E=0;
        for (int i =0; i < inlier_mask_E.rows;i++){
            std::cout<<"check: "<<inlier_mask_E.at<bool>(0,i)<<endl;
            if (inlier_mask_E.at<bool>(0,i)==1){
                sum_E++;
            }
        }

        int sum_H=0;
        for (int i =0; i < inlier_mask_H.rows;i++){
            std::cout<<"check: "<<inlier_mask_H.at<bool>(0,i)<<endl;

            if (inlier_mask_H.at<bool>(0,i)==1){
                sum_H++;
            }
        }
        std::cout<<"sum"<<sum_E<<sum_H<<endl;
