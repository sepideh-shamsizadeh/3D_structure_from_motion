...
//        descriptor->compute (img, features_[i], feats_colors_[i]);
            ...
            Mat outimg;
            cv::drawKeypoints( img, features_[i], outimg, Scalar::all(-1), DrawMatchesFlags::DEFAULT );
            imshow("ORB",outimg);
            waitKey(0);

            matcher->match ( descriptors_[i], descriptors_[j], matches,2 );
...
//
//
//        drawMatches ( img, features_[i], img, features_[j], matches, img_match );
